...
char sendId[10] = "ALL_MSG";
char sendBuf[CMD_SIZE];
char lcdLine1[17] = "Smart IoT By KSM";
...
void setup(){ 
  ...
  //pinMode(MOTOR_PIN, OUTPUT);   // 모터 핀을 출력으로 설정
  ...
  if (timerIsrFlag) //1초에 한번씩 실행
  {
    ...
    if (!(secCount % 5)) //5초에 한번씩 실행
    {
      ...
      if (cds >= 10)
      {
        char sendBufHere[20];
     //   cdsFlag = false;
        sprintf(sendBufHere, "[%s]CDS@%d\n", sendId, cds);
        client.write(sendBufHere, strlen(sendBufHere));
        client.flush();
...
void socketEvent()
{
  ...
  else if (!strcmp(pArray[1], "SERVO"))
  {
    myservo.attach(SERVO_PIN);
    myservoTime = secCount;
    int pos = atoi(pArray[2]);
    if (!strcmp(pArray[2], pos))
      myservo.write(pos);
    else
      myservo.write(pos);
    sprintf(sendBuf, "[%s]%s@%s\n", pArray[0], pArray[1], pArray[2]);
